...
const el = {
  qrcode: $('#qrcode'), qrcodeUrl: $('#qrcodeUrl'), qrcodeIps: $('#qrcodeIps'),
  qrcodeSection: $('.qrcode-section'), btnToggleQr: $('#btnToggleQr'),
...
function initQRCode() {
  ...
  el.qrcode.innerHTML = '';
  new QRCode(el.qrcode, { text: url, width: 140, height: 140, colorDark: '#1f2937', colorLight: '#ffffff' });
  el.qrcodeUrl.textContent = '扫码访问: ' + url;
}
el.btnToggleQr.addEventListener('click', () => {
  const s = el.qrcodeSection; s.classList.toggle('collapsed');
  el.btnToggleQr.textContent = s.classList.contains('collapsed') ? '展开二维码' : '收起二维码';
});
